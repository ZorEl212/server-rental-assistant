...
class BotManager:
    ...
    def __init__(self, routes, client=None):
        self.__client = TelegramClient("server_plan_bot", API_ID, API_HASH) if not client else client
        self.ROUTES: Dict = routes

    def start(self):
        self.__client.start(bot_token=BOT_TOKEN)
        self.__client.add_event_handler(self.command_handler, events.NewMessage(pattern="/"))
